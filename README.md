...
```bash
python vrp_benchmark.py --scenarios base alta_prioridade alta_demanda baixa_autonomia frota_reduzida maior --repetitions 10 --generations 1000
```
...
```bash
python vrp_benchmark.py --repetitions 10 --generations 1000 --output resultados.csv
```
...
```bash
python vrp_benchmark.py --scenarios base --repetitions 1 --generations 5 --output benchmark_smoke.csv
```
...
```bash
python -m unittest -q test_vrp.py
```

Para verificar a sintaxe dos módulos Python:

```bash
python -m py_compile main.py vrp.py tsp_base/tsp.py \
	vrp_domain.py vrp_evaluator.py vrp_repair.py vrp_genetic.py \
	vrp_benchmark.py vrp_scenarios.py test_vrp.py \
	tsp_base/__init__.py tsp_base/genetic_algorithm.py \
...
- [vrp.py](vrp.py): execução e visualização do VRP hospitalar.
- [vrp_domain.py](vrp_domain.py): modelos e carregamento dos dados.
- [vrp_genetic.py](vrp_genetic.py): algoritmo genético.
- [vrp_evaluator.py](vrp_evaluator.py): avaliação das rotas e restrições.
- [vrp_repair.py](vrp_repair.py): reparo de soluções inválidas.
- [vrp_feasibility.py](vrp_feasibility.py): diagnóstico de viabilidade dos cenários.
- [vrp_reporting.py](vrp_reporting.py): resumo estruturado das soluções para integração futura.
- [vrp_benchmark.py](vrp_benchmark.py): comparação entre estratégias.
- [test_vrp.py](test_vrp.py): testes automatizados.
